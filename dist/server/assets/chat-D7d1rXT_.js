import { y as getDefaultExportFromCjs, N as jsxRuntimeExports, X as reactExports } from "./server-Cgfy5VtR.js";
import { u as useLiveQuotes } from "./useLiveQuotes-a6H7Jnr0.js";
import { b as formatPrice, f as formatChangePct } from "./tickerConfig-7S1tvlm8.js";
...
import { g as getDailyUsage, C as CHAT_USAGE_KEY, b as bumpDailyUsage } from "./planUsage-C6qDsdHT.js";
...
const SUGGESTIONS = ["What is NIFTY 50 outlook today?", "Analyze Banking sector performance", "Should I buy RELIANCE for long-term?", "Explain my portfolio diversification", "Upcoming IPO opportunities in India"];
...
function ChatPage() {
  ...
  const [aiUnavailable, setAiUnavailable] = reactExports.useState(false);
  const [input, setInput] = reactExports.useState("");
  const [loading, setLoading] = reactExports.useState(false);
  const [usage, setUsage] = reactExports.useState(0);
  ...
  reactExports.useEffect(() => {
    ...
    supabase.from("portfolio").select("ticker,qty").eq("user_id", user.id).then(({
      data
    }) => setHoldings(data ?? []));
  }, [user]);
  const marketContext = reactExports.useMemo(() => {
    const nifty = quotes.find((q) => q.ticker === "NIFTY50");
    const sensex = quotes.find((q) => q.ticker === "SENSEX");
    const lines = [nifty ? `NIFTY 50: ${formatPrice(nifty.last)} (${formatChangePct(nifty.changePct)})` : "", sensex ? `SENSEX: ${formatPrice(sensex.last)} (${formatChangePct(sensex.changePct)})` : "", watchlist.length ? `Watchlist: ${watchlist.join(", ")}` : "", holdings.length ? `Portfolio: ${holdings.map((h) => `${h.ticker} x${h.qty}`).join(", ")}` : ""].filter(Boolean);
    return lines.join("\n");
  ...
  const remaining = limits.chatMessagesPerDay === Infinity ? Infinity : Math.max(0, limits.chatMessagesPerDay - usage);
  const send = async (text2) => {
    if (!text2.trim() || loading) return;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(PageShell, { title: "AI Market Assistant", subtitle: `Streaming · ${remaining === Infinity ? "Unlimited" : `${remaining} left today`} · ${profile?.plan ?? "free"} plan`, className: "max-w-4xl flex flex-col min-h-[calc(100dvh-8rem)] md:min-h-[calc(100dvh-4rem)]", actions: messages.length > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: clearChat, className: "px-3 py-1.5 rounded-md bg-secondary text-xs font-semibold hover:bg-secondary/70 transition flex items-center gap-1.5", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { className: "size-3.5" }),
    " Clear"
  ] }) : null, children: [
    aiUnavailable && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-3 mb-3 text-sm text-muted-foreground border border-warning/30", children: [
      aiNotConfiguredMessage(),
      " Add GEMINI_API_KEY or OPENAI_API_KEY to your .env file."
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card flex-1 min-h-[280px] p-4 overflow-y-auto mb-3 space-y-3", children: [
      messages.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center py-12", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Sparkles, { className: "size-10 text-primary mx-auto mb-3 opacity-60" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-muted-foreground mb-4", children: "Try one of these:" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-wrap gap-2 justify-center", children: SUGGESTIONS.map((s) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => send(s), className: "px-3 py-1.5 rounded-md bg-secondary text-sm text-foreground hover:bg-primary/15 hover:text-primary transition", children: s }, s)) })
      ] }),
      messages.map((m, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `flex ${m.role === "user" ? "justify-end" : "justify-start"}`, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `max-w-[85%] px-4 py-2 rounded-lg text-sm ${m.role === "user" ? "bg-primary/15 text-foreground border border-primary/30" : "bg-secondary text-foreground"}`, children: m.role === "assistant" ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "prose prose-sm prose-invert max-w-none [&_p]:my-1 [&_ul]:my-1 [&_ol]:my-1 [&_code]:bg-background/40 [&_code]:px-1 [&_code]:rounded", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Markdown, { remarkPlugins: [remarkGfm], children: m.content || "…" }) }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "whitespace-pre-wrap", children: m.content }) }) }, i)),
      loading && messages[messages.length - 1]?.role !== "assistant" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex justify-start", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-secondary px-4 py-2 rounded-lg text-sm text-muted-foreground animate-pulse", children: "Thinking…" }) }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { onSubmit: (e) => {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("input", { value: input, onChange: (e) => setInput(e.target.value), placeholder: "Ask about a stock, strategy, or indicator…", className: "flex-1 px-4 py-2 rounded-md bg-input border border-border focus:outline-none focus:border-primary" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "submit", disabled: loading, className: "px-4 py-2 rounded-md bg-primary text-primary-foreground font-semibold flex items-center gap-2 disabled:opacity-50", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Send, { className: "size-4" }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-muted-foreground text-center mt-3", children: "SEBI Disclaimer: Educational only. Not investment advice." })
  ] });
}
export {
