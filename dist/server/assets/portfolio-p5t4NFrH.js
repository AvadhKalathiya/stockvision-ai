import { R as React, y as getDefaultExportFromCjs, X as reactExports, N as jsxRuntimeExports } from "./server-Cgfy5VtR.js";
import { s as supabase } from "./client-T0sJvQy8.js";
import { w as useAuthStore, v as toast } from "./router-C3k8-z80.js";
import { d as getTickerConfig, T as TICKER_CONFIG, b as formatPrice, f as formatChangePct, A as ALL_TICKERS } from "./tickerConfig-7S1tvlm8.js";
import { u as useLiveQuotes } from "./useLiveQuotes-a6H7Jnr0.js";
import { P as PageShell } from "./PageShell-AvoXgHto.js";
import { Q as QueryError } from "./QueryState-D4yWYCiQ.js";
import { g as getLimits } from "./planLimits-DfVfhYvW.js";
import { S as Sparkles } from "./sparkles-BzGaueQV.js";
import { g as filterProps, M as require_baseExtremum, N as require_baseGt, O as require_baseIteratee, P as require_baseLt, y as isFunction, T as Text, K as polarToCartesian, c as Layer, s as getTickClassName, f as adaptEventsOfChild, L as Label, D as Dot, a as Curve, F as isNil, u as getValueByDataKey, S as Shape, A as Animate, l as get, w as interpolateNumber, x as isEqual, H as isNumber, b as LabelList, U as uniqueId, G as Global, I as mathSign, h as findAllByType, C as Cell, p as getMaxRadius, q as getPercentValue, a0 as warn, k as generateCategoricalChart, i as formatAxisMap, R as ResponsiveContainer, e as Tooltip } from "./generateCategoricalChart-DWEqI6XE.js";
import { c as clsx } from "./clsx-DgYk2OaC.js";
import { C as ChartPie } from "./chart-pie-_Q0-BHGH.js";
...
function usePortfolioData() {
  ...
  const load = reactExports.useCallback(async () => {
    ...
    setLoading(true);
    setError(null);
    const { data, error: err } = await supabase.from("portfolio").select("*").eq("user_id", user.id).order("created_at", { ascending: false });
    if (err) {
...
function PortfolioPage() {
  ...
  const [showAdvisor, setShowAdvisor] = reactExports.useState(false);
  ...
  const enriched = reactExports.useMemo(() => holdings.map((h) => {
  ...
  const totals = enriched.reduce((a, h) => ({
    value: a.value + h.value,
    cost: a.cost + h.cost,
    pnl: a.pnl + h.pnl
  }), {
    value: 0,
  ...
  const sectorMap = enriched.reduce((acc, h) => {
    const s = TICKER_CONFIG[h.ticker]?.sector || "Other";
    acc[s] = (acc[s] || 0) + (h.value > 0 ? h.value : h.cost);
    return acc;
  }, {});
  const pieData = Object.entries(sectorMap).map(([name, val]) => ({
    name,
    value: val
  })).sort((a, b) => b.value - a.value);
  const COLORS = ["oklch(0.87 0.2 165)", "oklch(0.75 0.18 220)", "oklch(0.75 0.18 70)", "oklch(0.65 0.25 25)", "oklch(0.65 0.2 290)", "oklch(0.7 0.05 235)"];
  const highestSector = pieData[0];
  const portfolioBase = totals.value > 0 ? totals.value : totals.cost;
  const maxExposurePct = highestSector && portfolioBase > 0 ? highestSector.value / portfolioBase * 100 : 0;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(PageShell, { title: "Portfolio", subtitle: "Track holdings with live Yahoo Finance P&L.", actions: /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: () => {
    if (!limits.canPortfolioAdvisor) {
      toast.error("AI Portfolio Advisor requires Student plan or higher");
      return;
    }
    setShowAdvisor(!showAdvisor);
  }, disabled: holdings.length === 0, className: "flex items-center gap-2 bg-accent text-accent-foreground px-4 py-2 rounded-lg text-sm font-bold hover:bg-accent/90 transition disabled:opacity-50", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Sparkles, { className: "size-4" }),
    " AI Advisor",
    !limits.canPortfolioAdvisor ? " 🔒" : ""
  ] }), children: [
    error ? /* @__PURE__ */ jsxRuntimeExports.jsx(QueryError, { message: error, onRetry: reload, label: "Failed to load portfolio" }) : null,
    quotesError ? /* @__PURE__ */ jsxRuntimeExports.jsx(QueryError, { message: "Live prices unavailable", onRetry: () => refetchQuotes(), label: "Quote feed error" }) : null,
    showAdvisor && holdings.length > 0 && limits.canPortfolioAdvisor && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "glass-card p-4 sm:p-6 mb-6 border-l-4 border-l-accent", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("h2", { className: "font-heading text-xl font-bold mb-4 flex items-center gap-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Sparkles, { className: "size-5 text-accent" }),
        " AI Portfolio Advisor"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-4 gap-4 sm:gap-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "md:col-span-1 space-y-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(ScoreCard, { label: "Health Score", value: healthScore.toFixed(0), highlight: healthScore > 75 ? "text-primary" : healthScore > 50 ? "text-warning" : "text-destructive", large: true }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(ScoreCard, { label: "Risk Score", value: `${riskScore}/100` }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(ScoreCard, { label: "Diversification", value: `${divScore.toFixed(0)}/100` })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "md:col-span-1 h-52 sm:h-64 flex flex-col items-center min-w-0", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-bold mb-2", children: "Sector Exposure" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(ResponsiveContainer, { width: "100%", height: "100%", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(PieChart, { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(Pie, { data: pieData, cx: "50%", cy: "50%", innerRadius: "55%", outerRadius: "75%", paddingAngle: 2, dataKey: "value", children: pieData.map((_, index) => /* @__PURE__ */ jsxRuntimeExports.jsx(Cell, { fill: COLORS[index % COLORS.length] }, `cell-${index}`)) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(Tooltip, { formatter: (val) => formatPrice(val), contentStyle: {
              background: "oklch(0.18 0.04 250)",
              border: "1px solid oklch(0.87 0.2 165 / 0.15)"
            } })
          ] }) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "md:col-span-2 space-y-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(InsightRow, { icon: ChartPie, title: "Sector Exposure", text: `${maxExposurePct.toFixed(1)}% allocated to ${highestSector?.name ?? "—"} stocks.` }),
          maxExposurePct > 40 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-3 bg-destructive/10 border border-destructive/30 rounded-lg flex items-start gap-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(ShieldAlert, { className: "size-5 text-destructive shrink-0 mt-0.5" }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-sm text-destructive", children: "Concentration Risk" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-sm text-destructive/90", children: [
                highestSector?.name,
                " exposure exceeds 40%. Consider rebalancing into FMCG or Pharma."
              ] })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(InsightRow, { icon: Sparkles, title: "AI Suggestion", text: holdings.length < 3 ? "Add 2–3 more sectors to improve diversification score." : "Add stable dividend yield (ITC.NS) or infrastructure picks to optimize risk-reward." })
        ] })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "Invested", value: formatPrice(totals.cost) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "Current Value", value: formatPrice(totals.value) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Metric, { label: "Unrealized P&L", value: `${formatPrice(totals.pnl)} (${formatChangePct(totalPct)})`, accent: totals.pnl >= 0 ? "text-primary" : "text-destructive" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { onSubmit: handleAdd, className: "glass-card p-4 sm:p-5 mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("select", { value: form.ticker, onChange: (e) => setForm({
        ...form,
        ticker: e.target.value
      }), className: "px-3 py-2 rounded-md bg-input border border-border text-foreground focus:outline-none focus:border-primary text-sm", children: ALL_TICKERS.map((t) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: t, children: t }, t)) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("input", { type: "number", step: "0.0001", placeholder: "Qty", value: form.qty, onChange: (e) => setForm({
        ...form,
        qty: e.target.value
      }), required: true, className: "px-3 py-2 rounded-md bg-input border border-border text-foreground focus:outline-none focus:border-primary text-sm" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("input", { type: "number", step: "0.01", placeholder: "Buy price", value: form.buy_price, onChange: (e) => setForm({
        ...form,
        buy_price: e.target.value
      }), required: true, className: "px-3 py-2 rounded-md bg-input border border-border text-foreground focus:outline-none focus:border-primary text-sm" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("input", { type: "date", value: form.buy_date, onChange: (e) => setForm({
        ...form,
        buy_date: e.target.value
      }), required: true, className: "px-3 py-2 rounded-md bg-input border border-border text-foreground focus:outline-none focus:border-primary text-sm" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { type: "submit", className: "px-4 py-2 rounded-md bg-primary text-primary-foreground font-semibold hover:opacity-90 transition flex items-center justify-center gap-2 text-sm", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "glass-card page-table-wrap", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "w-full min-w-[640px]", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { className: "bg-secondary/50", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: "text-left text-xs uppercase tracking-wider text-muted-foreground", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-right", children: "Buy" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-right", children: "Live" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-3 text-right", children: "Value" }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: loading ? /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { children: /* @__PURE__ */ jsxRuntimeExports.jsx("td", { colSpan: 7, className: "px-4 py-8 text-center text-muted-foreground", children: "Loading…" }) }) : enriched.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { children: /* @__PURE__ */ jsxRuntimeExports.jsx("td", { colSpan: 7, className: "px-4 py-8 text-center text-muted-foreground", children: "No holdings yet. Add your first position above." }) }) : enriched.map((h) => /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: "border-t border-border hover:bg-secondary/30 transition", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { className: "px-4 py-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-semibold font-mono-nums text-sm", children: h.ticker }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-muted-foreground truncate max-w-[140px]", children: h.company_name ?? TICKER_CONFIG[h.ticker]?.name })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-right font-mono-nums text-sm", children: Number(h.qty) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-right font-mono-nums text-sm", children: formatPrice(Number(h.buy_price), h.currency) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-right font-mono-nums text-sm", children: h.live ? formatPrice(h.current, h.currency) : "—" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-right font-mono-nums text-sm", children: formatPrice(h.value, h.currency) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { className: `px-4 py-3 text-right font-mono-nums text-sm font-semibold ${h.pnl >= 0 ? "text-primary" : "text-destructive"}`, children: [
          formatPrice(h.pnl, h.currency),
          /* @__PURE__ */ jsxRuntimeExports.jsx("br", {}),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs", children: formatChangePct(h.pnlPct) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-right", children: /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => handleDelete(h.id), className: "text-muted-foreground hover:text-destructive transition p-1", "aria-label": "Remove holding", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { className: "size-4" }) }) })
      ] }, h.id)) })
    ] }) })
  ] });
}
...
function ScoreCard({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-4 bg-secondary/50 rounded-lg border border-border text-center", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs uppercase text-muted-foreground mb-1", children: label }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `font-bold font-mono-nums ${large ? "text-3xl sm:text-4xl" : "text-xl sm:text-2xl"} ${highlight ?? ""}`, children: value })
  ] });
}
function InsightRow({
  icon: Icon,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-3 bg-secondary/30 rounded-lg border border-border flex items-start gap-3", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Icon, { className: "size-5 text-primary shrink-0 mt-0.5" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "font-bold text-sm", children: title }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm text-muted-foreground", children: text })
    ] })
  ] });
}
export {
